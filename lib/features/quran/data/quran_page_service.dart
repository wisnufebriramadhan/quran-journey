...
class QuranPageService {
  ...
  /// 🔥 CACHE halaman di memory: pageNumber -> List<QuranVerse>
  final Map<int, List<QuranVerse>> _pageCache = {};
  
  /// Service untuk handle offline data
  final QuranDownloadService _downloadService = QuranDownloadService();

  /// Fetch ayat berdasarkan nomor halaman (1-604)
  /// Priority: Memory Cache -> Local Storage -> API Online
  Future<List<QuranVerse>> fetchPage(int pageNumber) async {
    // ✅ 1. Cek memory cache terlebih dahulu (paling cepat)
    if (_pageCache.containsKey(pageNumber)) {
      return _pageCache[pageNumber]!;
    }

    // ✅ 2. Cek apakah data sudah di-download ke local storage
    final isDownloaded = await _downloadService.isQuranDownloaded();
    
    if (isDownloaded) {
      // Ambil dari local storage
      final verses = await _downloadService.getPageFromCache(pageNumber);
      if (verses.isNotEmpty) {
        // Simpan ke memory cache untuk akses berikutnya
        _pageCache[pageNumber] = verses;
        return verses;
      }
    }

    // ✅ 3. Fallback: Fetch dari API online
    try {
  ...
  /// 🔥 Clear memory cache (opsional)
  void clearCache() {
    _pageCache.clear();
  }

  /// Cek apakah data offline sudah tersedia
  Future<bool> isDataDownloaded() async {
    return await _downloadService.isQuranDownloaded();
  }
